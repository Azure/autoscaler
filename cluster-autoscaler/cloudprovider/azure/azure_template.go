...
func buildNodeFromTemplate(nodeGroupName string, inputLabels map[string]string, inputTaints string,
	...
	node.Labels = cloudprovider.JoinStringMaps(node.Labels, fetchLabel(template, node.Labels, inputLabels))
	klog.V(4).Infof("Setting node %s labels to: %s", nodeName, node.Labels)

	// Add the storage profile
	if template.VirtualMachineProfile != nil && template.VirtualMachineProfile.StorageProfile != nil &&
		...
		if template.VirtualMachineProfile.StorageProfile.OsDisk.DiskSizeGB != nil {
			resourceQuantity := *resource.NewQuantity(int64(int(*template.VirtualMachineProfile.StorageProfile.OsDisk.DiskSizeGB)*1024*1024*1024),
				resource.DecimalSI)
			node.Status.Capacity[apiv1.ResourceEphemeralStorage] = resourceQuantity
