...
func TestHasInstance(t *testing.T) {
	...
	mockVMSSVMClient.EXPECT().List(
		gomock.Any(),
		provider.azureManager.config.ResourceGroup,
		"test-asg",
		gomock.Any(),
	).Return(expectedVMSSVMs, nil).AnyTimes()
	...
	// Refresh cache
	_ = provider.azureManager.forceRefresh()
...
func TestUnownedInstancesFallbackToDeletionTaint(t *testing.T) {
	...
	unregisteredVMSSInstance := &apiv1.Node{
		...
		Spec: apiv1.NodeSpec{
			ProviderID: "azure:///subscriptions/sub/resourceGroups/rg/providers/Microsoft.Compute/virtualMachineScaleSets/unregistered-vmss-instance-id/virtualMachines/0", // nolint:lll
		},
	}
	...
	for _, orchMode := range orchestrationModes {
		t.Run(fmt.Sprintf("OrchestrationMode_%v", orchMode), func(t *testing.T) {
			...
			nodeNotInGroup := &apiv1.Node{
				Spec: apiv1.NodeSpec{
					ProviderID: "azure:///subscriptions/subscription/resourceGroups/test-resource-group/providers/Microsoft.Compute/virtualMachineScaleSets/test/virtualMachines/test-instance-id-not-in-group", //nolint:lll
				},
			}
