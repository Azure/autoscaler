...
func (scaleSet *ScaleSet) getScaleSetSize() (int64, error) {
	...
	// If the policy for this ScaleSet is Deallocate, the TargetSize is the capacity reported by VMSS minus the nodes
...
// instanceStatusFromProvisioningState used by orchestrationMode == compute.Flexible
func instanceStatusFromProvisioningState(provisioningState *string) *cloudprovider.InstanceStatus {
	if provisioningState == nil {
		return nil
	}

	klog.V(5).Infof("Getting vm instance provisioning state %s for %s", *provisioningState)
